...
def manhattan(rating1, rating2):
    ...
    for klucz in klucz1:
        if klucz in rating2.keys():
            porownanie=True
            distanceA=distanceA+abs(rating2[klucz]-rating1[klucz])
        if porownanie==True:
            return distanceA
        else:
            return -1
...
def computeNearestNeighbor(username, users):
    ...
    for username2 in users:
        if username2!=username:
            distance=0
            distance=manhattan(users[username],users[username2])
            distances.append((distance, username2))
    return sorted(distances)

    nameOfNearestNeighbor=sorted(distances)
